			...
			mediaMatch.add("(min-width: 992px)", () => {
				...
				function initVideoAnimations() {
					...
					const pinShared = {
						...
						pinReparent: false,
						invalidateOnRefresh: true,
						...(pinCtx ? { pinnedContainer: pinCtx } : {}),
					};

					// Single pin: pinning both .h1_hero_video and .h1-video-section caused
					// overlapping layers with the next section (e.g. services) under ScrollSmoother.
			...
			function sidebarStickyController() {
				...
				const pinCtx =
					...
					document.querySelector("#has_smooth")?.classList.contains("has-smooth")
						? smoothContent
						: undefined;
				const pinShared = pinCtx
					? {
							pinnedContainer: pinCtx,
							pinReparent: false,
						}
					: {};
